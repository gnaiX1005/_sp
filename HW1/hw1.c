...
// =========================================================
typedef struct {
    char op[16];     // 操作碼 (ADD, JMP, JMP_F, STORE...)
    char arg1[32];   
    char arg2[32];   
    char result[32]; 
} Quad;
...
void statement() {
    ...
    // --- 新增 WHILE 語法處理 ---
        ...
        next_token(); // 吃掉 while
        int start_pc = quad_count; // 記錄跳回來的起點
        next_token(); // 吃掉 (
        ...
        char back_pc[10]; sprintf(back_pc, "%d", start_pc);
        emit("JMP", "-", "-", back_pc); // 跳回開頭
        sprintf(quads[jmp_f_idx].result, "%d", quad_count); // 回填結束位置
    }
    else if (cur_token.type == TK_ID) {
        char name[32]; strcpy(name, cur_token.text); next_token();
...
typedef struct {
    ...
    int ret_pc; char ret_var[32]; int incoming_args[10]; int formal_idx;
} Frame;
...
void vm() {
    ...
    while (pc < quad_count) {
        ...
        if (strcmp(q.op, "FUNC_BEG") == 0) {
            ...
            int p_count = atoi(q.arg2); int t_pc = -1;
            for (int i = 0; i < f_count; i++) if (strcmp(func_names[i], q.arg1) == 0) t_pc = func_pc[i];
            ...
            param_sp -= p_count; pc = t_pc; continue;
        }
        else if (strcmp(q.op, "FORMAL") == 0) set_var(q.arg1, stack[sp].incoming_args[stack[sp].formal_idx++]);
        else if (strcmp(q.op, "RET_VAL") == 0) {
            int rv = get_var(q.arg1); int ra = stack[sp].ret_pc;
            char tv[32]; strcpy(tv, stack[sp].ret_var);
            sp--; set_var(tv, rv); pc = ra; continue;
        }
        pc++;
    }
